...
from utils.mappings import dep_to_crs, name_dep_to_num_dep, num_dep_to_name_dep
from utils.satellite_image import SatelliteImage
from utils.utils import get_environment


def crs_to_gps_image(
    satellite_image: SatelliteImage = None, filepath: str = None
):
    """
    Gives the gps point of the left-top boundingbox of the image. These bounds
    are found in the filename (quicker than if we have to open all the images).
    So this method is based on the filenames of the pleiades images.
    Argument is either a SatelliteImage or a filepath.
...
def gps_to_crs_point(lat: float, lon: float, crs: int):
    ...
    """
    # Convert GPS coordinates to coordinates
    # in destination coordinate system (CRS)
    transformer = Transformer.from_crs(
...
def find_image_of_point(coordinates: List, folder_path: str, coord_gps=False):
    ...
    Args:
        ...
        folder_path (str): the path of the folder in which
        we search the image containing the point
        coord_gps (boolean): specifies if the coordinate
        is a gps coordinate or not
...
def find_image_different_years(
    ...
    Returns:
        The path of the image representing the same place
    ...
    dpt_base = split_folder[4].lower()
    # dep_num_base = name_dep_to_num_dep[departement_base]
    path = environment["local-path"]["PLEIADES"][different_year][dpt_base]
    folder_path = "../" + path
    ...
    if new_filename in os.listdir(folder_path):
        ...
        return "There is no image of this place in the requested \
            year in the database Pléiades."
